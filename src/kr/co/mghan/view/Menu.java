...
public class Menu extends CommonMethod
{
		...
		while (top_menu_bool)
		{
			...
			{ // 특정인물 조회
				...
				while (code)
				{
					...
					String s_code = new SearchHR().emp_View(eb);
	...
	public void fifth_menu()
	{
		...
		while (dept_menu_bool)
		{
			...
			{
				new DeptMenu().dept_search(ar_db);
			}
			else if (menu.equals("2"))
			{
				ar_db = new DeptMenu().dept_add(ar_db);
			}
			else if (menu.equals("3"))
			{
				ar_db = new DeptMenu().dept_mod(ar_db);
			}
			else if (menu.equals("4"))
			{
				ar_db = new DeptMenu().dept_del(ar_db);
			}
		}
	}
